...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden md:flex w-64 bg-white border-r border-gray-100 min-h-screen flex-col">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 safe-bottom">
        <div className="flex justify-around items-center h-16">
          ...
          {session ? (
            <button
              onClick={() => signOut()}
              className="flex flex-col items-center justify-center gap-0.5 flex-1 h-full text-emerald-500"
            >
              <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M5 13l4 4L19 7" />
              </svg>
              <span className="text-[10px] font-medium">連携済</span>
            </button>
          ) : (
            <button
              onClick={() => signIn("google")}
            ...
            >
              ...
              <span className="text-[10px] font-medium">連携</span>
            </button>
          )}
        </div>
      </nav>
